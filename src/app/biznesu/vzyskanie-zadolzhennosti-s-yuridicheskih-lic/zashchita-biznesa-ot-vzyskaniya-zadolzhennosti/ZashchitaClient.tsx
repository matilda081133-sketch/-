...
export default function ZashchitaClient() {
  ...
  const situations: { tag: string; title: React.ReactNode; desc: React.ReactNode; btnText: string; btnHref: string }[] = [
    {
      tag: 'Досудебный порядок',
      title: 'Претензия',
      desc: 'Проверим срок ответа, договорный порядок, расчёт, приложения и последствия признания требований.',
      btnText: 'Проверить претензию',
      btnHref: '#documents'
    },
    {
      tag: 'Арбитражный процесс',
      title: 'Исковое заявление',
      desc: 'Проверим суд, номер дела, срок отзыва, дату заседания, расчёт и доказательства кредитора.',
      btnText: 'Передать иск',
      btnHref: '#documents'
    },
    {
      tag: 'Приказное производство',
      title: 'Судебный приказ',
      desc: 'Проверим дату получения, применимый порядок и срок подачи возражений.',
      btnText: 'Проверить срок',
      btnHref: '#documents'
    },
    {
      tag: 'Ограничения счетов',
      title: 'Обеспечительные меры',
      desc: 'Суд по заявлению истца принял обеспечительные меры и ограничил операции по расчётным счетам. Проверим основания и соразмерность ограничений, подготовим ходатайство об отмене или замене мер.',
      btnText: 'Оценить меры',
      btnHref: '#urgent'
    },
    {
      tag: 'Судебный акт',
      title: 'Решение суда',
      desc: 'Проверим дату принятия и получения, вступление в силу, сроки и основания обжалования.',
      btnText: 'Проверить решение',
      btnHref: '#documents'
    },
    {
      tag: 'Исполнительное производство',
  ...
  const urgentItems: { title: React.ReactNode; desc: string }[] = [
    {
      ...
      desc: 'Если отзыв и доказательства не представлены в установленный судом срок, дело может быть рассмотрено по имеющимся материалам. Это уменьшает возможность своевременно оспорить расчёт, представить доказательства и заявить встречные требования.'
    },
    {
    ...
    },
    {
      ...
      desc: 'Публикация о намерении обратиться с заявлением о банкротстве требует проверить основания требования, срок реакции, финансовое состояние компании и риски для руководителя. Само уведомление не означает автоматического банкротства или субсидиарной ответственности.'
    }
  ];
  ...
  const routes: { tag: string; title: React.ReactNode; desc: string; route: string }[] = [
    ...
    },
    {
      ...
      route: 'Добиться утверждения приемлемого графика платежей и отмены либо замены обеспечительных мер'
    }
  ];
  ...
  const stages: { num: string; stage: React.ReactNode; works: React.ReactNode; label?: string; result: string }[] = [
    ...
    },
    {
      ...
      label: 'Результат этапа',
      result: 'Если долг подтверждается, добиваемся приемлемых условий его погашения: снижения дополнительной нагрузки, отсрочки, рассрочки или мирового соглашения.'
    },
    {
      num: '04',
      stage: <>Защита в арбитражном<br />суде всех инстанций</>,
      works: 'Защищаем счета и имущество компании: проверяем основания ограничений и добиваемся отмены или замены несоразмерных обеспечительных мер. Выявляем встречные требования к кредитору и используем их для уменьшения взыскиваемой суммы, зачёта или предъявления встречного иска.',
      label: 'Цель этапа',
      result: 'Добиться полного или частичного отказа во взыскании необоснованной суммы, снизить неустойку и исключить требования, которые кредитор не подтвердил документами.'
    },
    {
      num: '05',
      ...
      label: 'Результат этапа',
      result: 'Сохранение платёжеспособности бизнеса, снятие ограничений со счетов и подготовка оптимального порядка выплат.'
    }
  ];
  ...
  const claims: { tag: string; title: React.ReactNode; desc: string }[] = [
    ...
    },
    {
      ...
      desc: 'Проверка расчёта процентов и комиссий, условий договора, объёма ответственности поручителей и залогодателей компании.'
    },
    {
      tag: 'Авансы',
  ...
  const faqs = [
    ...
    },
    {
      ...
      a: (
        <>
          Нужно установить источник ограничения. Если это обеспечительная мера суда, подаётся ходатайство об отмене или замене мер. Если исполнительный лист уже находится в банке или в ФССП, задача передаётся в направление{' '}
    ...
    {
      q: 'Защищаете ли вы компанию при угрозе банкротства?',
      a: (
        <>
          Да, но это самостоятельный банкротный проект. Если кредитор намерен инициировать банкротство или финансовое состояние компании указывает на неплатёжеспособность, оцениваем риски и передаём задачу в направление{' '}
          <Link href="/biznesu/bankrotstvo-biznesa/" style={{ color: 'var(--color-primary)', textDecoration: 'underline' }}>
            «Банкротство бизнеса»
          </Link>.
        </>
      )
    },
    {
      ...
      a: 'Да, если характер и стадия дела позволяют. Анализ документов, подготовку позиции и значительную часть коммуникации можно организовать дистанционно. Возможность онлайн-участия в заседаниях зависит от суда и обстоятельств дела.'
    }
  ];
      ...
      <MilitaryHero
        ...
        subtitle={
          <span style={{ display: 'inline-block', maxWidth: '750px', textWrap: 'balance' }}>
            Проверим обоснованность требований кредитора, расчёт долга и первичные документы. Подготовим контррасчёт, возражения или встречный иск, защитим компанию в арбитражном суде, снизим завышенную неустойку и добьёмся отмены или замены необоснованных обеспечительных мер.
          </span>
        }
        ...
        trustItems={[
          ...
          { text: 'Снижаем завышенную неустойку по ст. 333 ГК РФ' },
        ...
        <div className="container">
          <div style={{ maxWidth: '800px', marginBottom: '36px' }}>
            ...
            <p style={{ color: 'rgba(255,255,255,0.85)', fontSize: '16px', marginBottom: '40px', maxWidth: '720px', lineHeight: 1.6 }}>
              В арбитражном споре промедление может привести к рассмотрению дела без позиции компании, принятию обеспечительных мер и ограничениям в хозяйственной деятельности.
            </p>
          </div>
          ...
          <div style={{
            ...
            <span style={{ display: 'inline-block' }}>
              Сначала оцениваем правовую позицию, добиваемся отмены или замены необоснованных обеспечительных мер и снижаем размер требований кредитора.
            </span>
          </div>
