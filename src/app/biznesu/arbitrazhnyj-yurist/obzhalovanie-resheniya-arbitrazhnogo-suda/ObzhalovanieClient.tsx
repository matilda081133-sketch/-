...
export default function ObzhalovanieClient() {
    ...
    '@graph': [
      ...
      },
      {
        ...
        'mainEntity': [
                    ...
                    {
                              "@type": "Question",
                              "name": "Куда подаётся апелляционная жалоба на решение АС Липецкой области?",
                              "acceptedAnswer": {
                                        "@type": "Answer",
                                        "text": "Жалоба адресуется в Девятнадцатый арбитражный апелляционный суд (г. Воронеж), но подаётся через Арбитражный суд Липецкой области (ст. 257 АПК РФ)."
                              }
                    },
                    {
                              "@type": "Question",
                              "name": "Куда подаётся кассационная жалоба?",
                              "acceptedAnswer": {
                                        "@type": "Answer",
                                        "text": "Кассационная жалоба подаётся в Арбитражный суд Центрального округа (г. Калуга) через суд первой инстанции, принявший первоначальное решение."
                              }
                    },
                    {
                              "@type": "Question",
                              ...
                              "acceptedAnswer": {
                                        "@type": "Answer",
                                        "text": "Подадим ходатайство о приостановлении исполнения при наличии предусмотренных АПК РФ оснований и приложим необходимые обоснования или встречное обеспечение. Решение по ходатайству принимает суд."
                              }
                    },
                    {
                              "@type": "Question",
                              "name": "Что делать, если срок обжалования пропущен?",
                              "acceptedAnswer": {
                                        "@type": "Answer",
                                        "text": "Если срок пропущен по уважительной причине (позднее изготовление решения, сбои в почтовой доставке), одновременно с жалобой подаётся ходатайство о восстановлении пропущенного срока."
                              }
                    }
          ]
      }
  ...
  const urgentItems = [
    {
      title: (
        <span>
          <span style={{ display: 'inline-block' }}>Общий срок апелляции:</span> <br />
          <span style={{ display: 'inline-block' }}>1 месяц</span>
        </span>
      ),
      desc: 'Срок обычно исчисляется со дня принятия решения в полном объёме. Проверим дату начала течения срока и основания для его восстановления при пропуске.'
    },
    {
      title: (
        <span>
          <span style={{ display: 'inline-block' }}>Общий срок кассации:</span> <br />
          <span style={{ display: 'inline-block' }}>2 месяца</span>
        </span>
      ),
    ...
    },
    {
      title: (
        <span>
          <span style={{ display: 'inline-block' }}>Ограничение на новые</span> <br />
          <span style={{ display: 'inline-block' }}>доказательства в апелляции</span>
        </span>
      ),
      desc: 'Апелляционный суд принимает дополнительные доказательства, когда обоснована невозможность представить их в первой инстанции по причинам, не зависящим от стороны.'
    },
    {
      title: (
        <span>
    ...
    },
    {
      ...
      desc: 'Вступивший в силу судебный акт может повлиять на доказывание обстоятельств в последующих делах с участием тех же лиц. Проверим этот риск при анализе решения.'
    }
  ];
  ...
  const stages = [
    ...
    },
    {
      ...
      result: 'Мотивированное ходатайство о приостановлении исполнения подано; итоговое решение по ходатайству принимает суд.'
    },
    {
      num: '04',
      ...
      result: 'Жалоба зарегистрирована; получение определения о её принятии контролируем и при необходимости устраняем указанные судом недостатки.'
    },
    {
      num: '05',
      ...
      result: 'Доводы жалобы представлены и защищены в заседании; после принятия постановления определяем дальнейшие действия по делу.'
    }
  ];
  ...
  const faqs = [
    ...
    },
    {
      ...
      a: 'Подадим ходатайство о приостановлении исполнения при наличии предусмотренных АПК РФ оснований и приложим необходимые обоснования или встречное обеспечение. Решение по ходатайству принимает суд.'
    },
    {
