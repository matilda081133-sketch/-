          ...
          <div className="carousel-container" ref={scrollRef} style={{ display: 'flex', alignItems: 'stretch', gap: '24px', overflowX: 'auto', scrollBehavior: 'smooth', paddingBottom: '16px' }}>
            {team.map((person, index) => (
              <div key={index} className="card team-card carousel-item" style={{ 
                ...
                <p style={{ color: 'var(--color-text-secondary)', fontSize: '13px', margin: '0 0 16px 0', flexGrow: 1, lineHeight: 1.5 }}>{person.desc}</p>
                {person.link && (
                  <Link href={person.link} style={{ fontSize: '13px', color: 'var(--color-primary, #10273B)', fontWeight: 600, textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: '4px', marginTop: 'auto' }}>
