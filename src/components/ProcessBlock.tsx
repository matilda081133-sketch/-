...
export default function ProcessBlock({
  ...
  // Сценарий 0: 6 регулярных шагов + 1 баннер (7 элементов всего) -> Ряд 1: 4 шага (01,02,03,04) в grid-4; Ряд 2: 2 шага (05,06) + Баннер (span 2) в grid-4
  const is6Plus1Banner = regularSteps.length === 6 && bannerSteps.length === 1;
    ...
    <section className="section bg-light" style={{ position: 'relative', overflow: 'hidden', padding: '80px 0', background: 'var(--color-white)' }}>
      ...
      <div className="container" style={{ position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ position: 'relative' }}>
          {/* СЦЕНАРИЙ 7+1: 7 регулярных шагов + 1 баннер (Ряд 1: 4 шага 01-04 в grid-4; Ряд 2: 3 шага 05-07 + Баннер в grid-4) */}
          {is7Plus1Banner ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '40px' }}>
              ...
              {/* Ряд 2: 3 этапа 05 -> 06 -> 07 + Карточка «Дистанционный формат» в один ряд (grid-4) */}
              <div className="grid grid-4" style={{ gap: '24px', alignItems: 'stretch' }}>
                {regularSteps.slice(4, 7).map((step, i) => (
            ...
            /* СЦЕНАРИЙ 4: 7 регулярных этапов без баннера (Ряд 1: 3 этапа 01-03 в grid-3; Ряд 2: 4 этапа 04-07 в grid-4 в один ряд) */
            <div style={{ display: 'flex', flexDirection: 'column', gap: '40px' }}>
              {/* Ряд 1: 3 этапа 01 -> 02 -> 03 */}
              <div className="grid grid-3" style={{ gap: '40px' }}>
                {steps.slice(0, 3).map((step, i) => (
              ...
              {/* Ряд 2: 4 этапа 04 -> 05 -> 06 -> 07 в один ряд */}
              <div className="grid grid-4" style={{ gap: '24px' }}>
                {steps.slice(3, 7).map((step, i) => (
                  <div key={i + 3} className="stage-item" style={{ 
                    position: 'relative',
                    display: 'flex',
                    flexDirection: 'column',
