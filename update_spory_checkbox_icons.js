...
const clientFile = 'src/app/grazhdanam/zemelnyj-yurist/spory-o-granicah-zemelnogo-uchastka/SporyOGranicahClient.tsx';
let content = fs.readFileSync(clientFile, 'utf-8');

const oldBlock4 = /\{\/\* ═══ 5\. ДОКУМЕНТЫ ДЛЯ ПРОВЕРКИ ГРАНИЦЫ ═══ \*\/\}[\s\S]*?\{\/\* ═══ 6\. ПРАКТИКА ПО СПОРАМ О ГРАНИЦАХ ═══ \*\/\}/m;

const newBlock4 = `{/* ═══ 5. ДОКУМЕНТЫ ДЛЯ ПРОВЕРКИ ГРАНИЦЫ ═══ */}
...
content = content.replace(oldBlock4, newBlock4);
fs.writeFileSync(clientFile, content, 'utf-8');
console.log('SporyOGranicahClient block 4 updated with checkbox icons');
