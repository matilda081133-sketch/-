...
// 3. Fix Urgent Block alignment
const urgentHeaderRegex = /<h2 className="section-title text-center" style=\{\{ marginBottom: '24px', textAlign: 'center' \}\}>\s*Если помощь нужна прямо сейчас\s*<\/h2>/;
const newUrgentHeader = `<h2 className="section-title" style={{ marginBottom: '24px', textAlign: 'left' }}>
            Если помощь нужна прямо сейчас
          </h2>`;
if (content.match(urgentHeaderRegex)) {
  content = content.replace(urgentHeaderRegex, newUrgentHeader);
  console.log('Fixed urgent header alignment');
}

const urgentPRegex = /<p style=\{\{ textAlign: 'center', color: 'var\(--color-text-secondary\)', fontSize: '16px', maxWidth: '700px', margin: '0 auto 40px auto', lineHeight: 1\.6 \}\}>\s*Ниже — типичные ситуации, в которых откладывать обращение за юридической помощью опасно\. Чем раньше подключится профильный специалист, тем больше возможностей для защиты ваших прав\.\s*<\/p>/;
const newUrgentP = `<p style={{ textAlign: 'left', color: 'var(--color-text-secondary)', fontSize: '16px', maxWidth: '800px', marginBottom: '40px', lineHeight: 1.6 }}>
...
if (content.match(urgentPRegex)) {
  content = content.replace(urgentPRegex, newUrgentP);
  console.log('Fixed urgent p alignment');
}
